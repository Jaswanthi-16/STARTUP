...
export default function StatsCard({ title, value, icon: Icon, change, color }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 dark:bg-slate-900 rounded-xl shadow-sm p-6 border border-slate-100 dark:border-slate-800 dark:border-slate-800/80 flex flex-col justify-between h-full transition-colors duration-200">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400 dark:text-slate-400">{title}</h3>
        <div className={`p-2 rounded-lg bg-slate-50 dark:bg-slate-900 dark:bg-slate-800 ${color}`}>
          <Icon className="w-5 h-5" />
        </div>
      </div>
      <div>
        <p className="text-3xl font-bold text-slate-900 dark:text-white dark:text-white">{value}</p>
        <div className="flex items-center mt-2">
          <span className={`text-sm font-medium ${isPositive ? 'text-green-600' : 'text-red-500'}`}>
            {isPositive ? '+' : ''}{change}%
          </span>
          <span className="text-sm text-slate-400 dark:text-slate-500 ml-2">vs last month</span>
        </div>
      </div>
    </div>
  );
