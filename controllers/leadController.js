 ...
 * @returns {Object} Paginated response containing leads
 */
export const getLeads = async (req, res, next) => {
  try {
    ...
    // Build filter object dynamically, isolating by owner
...
export const createLead = async (req, res, next) => {
  try {
    const leadData = {
      ...req.body,
      owner: req.user._id, // Enforce current user as owner
    };

    const newLead = await Lead.create(leadData);

    return successResponse(res, { lead: newLead }, 'Lead created successfully', 201);
  } catch (error) {
    next(error);
  }
};
 ...
 * @param {Object} res - Express response object
 * @param {Function} next - Express next middleware function
 * @returns {Object} The requested lead
 */
 ...
 * @param {Object} res - Express response object
 * @param {Function} next - Express next middleware function
 * @returns {Object} Stats representing total, breakdown, and conversion rate
 */
 ...
 * @param {Object} res - Express response object
 * @param {Function} next - Express next middleware function
 * @returns {Array} Array of monthly stats
 */
export const getMonthlyStats = async (req, res, next) => {
  try {
    ...
    // Merge DB results into full array
 ...
 * @param {Object} res - Express response object
 * @param {Function} next - Express next middleware function
 * @returns {Array} List of minimal lead objects
 */
